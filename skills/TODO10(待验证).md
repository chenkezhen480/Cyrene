...
## 20. 语音实现流程审计与修复计划（2026-09-10，14212a1 工具化重构后）

> 本节为多 agent 审计（Map → 对抗验证 → 完整性检查）的核实结论，**取代上文 Phase 5-8 与第 19 节中的旧语音方案**（流式 TTS、独立语音 SSE 事件、"ASR/TTS 不注册为 Tool"）——14212a1 已删除独立语音事件与 `/api/audio/*` 端点（含 `AudioCapabilityHandler`/`AudioTranscriptionHandler`/`SpeechChunker`/`VoiceOutputCoordinator`），改为普通 ReAct 工具 + 通用 `tool_output` 事件。旧节中 `/api/audio/*` 端点记录（812-814、1034-1036 行）已失效。

### 20.1 当前流程（已验证）

```text
浏览器录音 (MediaRecorder, app.js:438-504, 候选 mime audio/webm;codecs=opus → audio/webm → audio/ogg)
  → POST /api/files/upload (FileUploadHandler) → knowledge-uploads/input/<uuid>.audio.webm
  → /files/input/... 作为 context.File 发 /api/chat (attachments 恒为 [])
  → AgentPromptBuilder:60-65 注入 "[Audio File: ...] Use the transcribe_audio tool"
  → transcribe_audio {file} → AgentToolRuntime.loadAudioSource:379-426 (/files/ 或 /api/artifacts/{id})
       → 空/大小/MIME 门禁 (AudioTranscriptionTool:70-102)
       → POST {voice.baseUrl}/audio/transcriptions (whisper-1, multipart)
       → ToolOutput.text(全文给模型)；SSE tool_output 只见前 100 code points
  → synthesize_speech {text, voice?} → POST {voice.baseUrl}/audio/speech (tts-1, mp3)
       → Artifact speech-<uuid>.mp3 (sessionId=null) → ToolOutput.artifacts
       → SSE tool_output.artifacts → 前端 <audio src="/api/artifacts/{id}/preview">
```

- 注册门禁：`voice.provider=none`/空 → 移除两个工具（`AgentToolRuntime.prepareVoiceTools:315-343`）；工具实例持稳定 `VoiceDelegate`（`ModelProviderRuntime:135-157`），热更新按调用时分发当前 generation；热重载仅当 8 个 `voice.*` 键变化时重建。
- 配置唯一来源 `model.conf` `voice.*` 8 键（Web UI "voice" 节）；`HARNESS_MODEL_VOICE_*` 仅首次缺 model.conf 时一次性迁移；voice **无** chat 组回退，仅接受 `openai|none|blank`。
- `<uuid>.audio.webm` 双扩展名是有效契约（`FileUploadHandler:70-78` 写、`AgentPromptBuilder.isAudioFile:209-225` 读），有测试钉住。
- `FallbackChatModel` AudioContent 内联转写分支当前不可达（全仓库无代码构造 langchain4j `AudioContent`，且流式路径绕过该包装）。
- TTS 产物可回灌 ASR：`/api/artifacts/{id}` 是 `transcribe_audio` 合法引用。

### 20.2 Major 问题

- [ ] **鉴权缺失**：`POST /api/files/upload`、`/files/*` 静态挂载（整棵 knowledge-uploads 树，含知识库对象存储 `objects/`、`.staging/`）、`/api/artifacts/{id}`、`/{id}/preview`、`/api/artifacts/session/{sessionId}`（响应含绝对磁盘路径）全部无鉴权（`Main.java:170-176,201-202,441-447`；`ArtifactHandler` 从不调用 `ApiRequestAuthenticator`，全服无 `app.before`）。注意：前端 `<audio>` 无 token 播放正依赖此缺口，修复需选方案（签名短时 URL / Bearer+fetch blob / cookie）。
- [ ] **格式支持被入口截断**：上传白名单音频仅 mp3/wav/ogg(+audio webm)（`FileUploadHandler:27-37`），`.m4a` 直接 400 而 ASR 支持 audio/mp4；`detectAudioMimeType:429-432` probe 结果优先于扩展映射（主机给出 audio/x-m4a 即永久拒绝）；`python_sandbox` 仅 .mp3/.wav 产出可用作 ASR 输入；`FfmpegTool` 不产 artifact 无法转码接力。端到端实际可达格式仅 {mp3, wav, ogg, audio-webm}，且白名单无配置项可扩展。
- [ ] **Safari/iOS 录音扩展名错位**：app.js:485 仅 `includes('ogg') ? 'ogg' : 'webm'`，Safari 默认 `audio/mp4` 被命名 `.webm` 并按 `audio/webm` 申报给 ASR（靠 Whisper 嗅探内容才碰巧能用，存储名/MIME 均错）。
- [ ] **畸形工具参数 JSON 杀死整个 SSE 流**：`ReActEngine:702-709` 容忍坏 JSON 继续循环，但 `ChatHandler.parseToolArguments:290-295` 抛 IOException → 被当作流写入失败 → `token.cancel()` → 用户收到假 "cancelled"，整轮丢失。
- [ ] **记忆压缩删除语音产物**：`AgentMemoryRuntime.stripToolMessages` → `deleteToolMessages` 删掉持久化 role-tool 消息——唯一完整 ASR 转录副本与 TTS ARTIFACT block；会话回放同样丢失 `/api/artifacts/{id}` 引用，音频文件却永留磁盘；trace 内另有整份无上限副本（`ReActStep.toolResults` 含全文与服务器绝对路径，无脱敏）。

### 20.3 Minor 问题（精选）

- [ ] 提示语与目录脱节：`AgentPromptBuilder:60-65` 无条件注入 transcribe_audio 指引，不查 run catalog——`voice.provider=none` 时模型被服务端提示调用不存在的工具。
- [ ] TTS/image/video artifact `sessionId=null`（根因：应用级注册表无会话绑定，`AgentToolRuntime.java:338` 裸方法引用）→ session artifact 列表永远为空、无保留策略、磁盘无限增长。
- [ ] UI 转录只见 100 code points（`ToolOutputSseMapper:13`，模型可见全文、重载会话可见，live 通道截断）且转录文本从不进输入框（前端只塞静态 `voiceInputPrompt`）。
- [ ] 取消无法中断进行中 ASR/TTS：两工具非 `CancellableTool`，OkHttp 忽略 interrupt，最长阻塞 120s（`ImageGenerationTool:31` 已有实现可参照）。
- [ ] 报错不可自纠：超限错误不含阈值数字；引用解析错误双前缀包裹 `Tool [transcribe_audio]: Audio transcription failed: ...`；空转录 `ToolOutput.text("")` 与静默无法区分，模型可能重复调用。
- [ ] 3 倍全量缓冲：`readAllBytes` + 两次 `AudioSource` 克隆后才校验大小/MIME，size 校验做两遍（工具 + provider）；默认 100MB artifact ≈ 300MB 堆。
- [ ] TTS 输出硬编码三处（`response_format "mp3"`、`.mp3` 后缀、`audio/mpeg`），`VoiceCapabilities.outputFormats()` 无任何消费者——换 provider 即静默错标。
- [ ] `createAll` 一票否决：一个 voice/realtime 错值令 AgentOrchestrator 构造失败、进程起不来（Web 热更新路径有 400+回滚，手改 model.conf 无保护）；`voice.asrMaxSizeMb≥2048` 时 `multiplyExact` 抛 ArithmeticException 同样炸启动。
- [ ] `voice.timeoutSeconds` 不管 write timeout（OkHttp 默认 10s，20MB 上传可能先撞它）。
- [ ] `spawn_subagent` 广告列表含不存在的 `code_sandbox`（实为 `python_sandbox`）；voice 工具可授权给子代理（completion_contract 支持 AUDIO artifact）但不在广告列表，模型无从得知。
- [ ] 前端：上传失败 toast 显示字面 `undefined`（chips 无 `name` 字段，app.js:721 vs 487）；失败附件被静默丢弃不恢复；空录音（0 字节）照常上传浪费一轮 ReAct；Enter 绕过 isRecording 守卫；硬编码中文 toast（app.js:728）；重复文件去重比较不存在的字段永不相等。
- [ ] `tool_output` 无 status 字段：丢序/丢 `tool_call_created` 时前端 `canTransition(null, undefined)` 抛 'Unknown tool status: undefined'，整条流中断。
- [ ] `UrlDownloader` SSRF 防护仅验首跳（followRedirects(true)），重定向可绕过私网封锁；下载根默认 `uploads` 与 `/files/` 根 `./knowledge-uploads` 不一致，URL 下载的音频永远无法经 /files/ 引用。
- [ ] `/api/structured-output` 同样可调 voice 工具，但响应 `{data, meta}` 无 artifacts 字段，TTS 产物对调用方完全不可见（且 sessionId=null 无人回收）。
- [ ] 知识库上传音频（`/api/knowledge/upload` 无扩展名门禁）→ ingest 失败 "No Markdown content converted"，无 ASR-on-ingest 路径，也无引导性拒绝。

### 20.4 死代码 / 遗留

- [ ] `RealtimeModelProvider` 全链孤儿：接口 + `NoOpRealtimeModelProvider` + `RealtimeDelegate` + `AgentOrchestrator.realtimeModel():651`（零调用者）+ Web UI "realtime" 配置节（`ModelConfigKey.java:50-52,89-90`；设任何非 none 值炸 provider set）。
- [ ] 空目录 `harness-agent/src/main/java/com/harness/agent/voice/` 与对应 test 目录。
- [ ] `FallbackChatModel` AudioContent 分支（休眠且吞错误为 `[Audio could not be processed]`，违反"不得吞错"不变量）。
- [ ] `ChatRequest.attachments` 音频路径：`AttachmentType.AUDIO` 被解析后直接丢弃（`MultimodalParser:173` 分类、无消费者、无门禁、无提示）——或补全处理，或显式拒绝。
- [ ] `VoiceModelProvider.timeoutSeconds()` 与 `VoiceCapabilities.outputFormats()` 为惰性契约面（无主源码消费者）。
- [ ] `Icons.volume`（app.js:127，旧独立语音播放 UI 残留）；`.env.example` 的 `HARNESS_RAG_EMBED_DIM` 死文档（实际维度来自 model.conf `embedding.dimension`）。

### 20.5 文档漂移

- [ ] README/README_EN 无语音功能描述，API 表缺 `/api/files/upload`、artifact 端点、model-config 端点；仍宣称 Realtime 能力（实际仅 NoOp，任何值不可用）。
- [ ] 本文件旧节仍记录已删除的 `/api/audio/*` 端点与 Handler（见节首说明）。
- [ ] AGENTS.md 未提 `transcribe_audio`/`synthesize_speech` 两工具与 `voice.*` 配置。
- [ ] `.env.example` 无任何 voice 键；EnvKey 有 30 个键未记录（含 docker-compose 在用的 `HARNESS_TOOL_BROWSER_WORKER_URL`）。

### 20.6 测试缺口

- [ ] `AudioTranscriptionToolTest`/`SpeechSynthesisToolTest` 各仅 1 个 happy-path 测试；补门禁、缺参、空音频、超限、MIME 拒绝、错误包裹、空转录分支。
- [ ] 缺 `OpenAiVoiceModelProviderTest`（ASR multipart 线格式、text 字段解析、错误体截断、TTS JSON 体、超时配置）。
- [ ] 缺 `FileUploadHandlerTest`（白名单、400、`.audio` 标记、空扩展名绕过）。
- [ ] 缺 `ArtifactStorageServiceTest`/`ArtifactHandlerTest`（sessionId 持久化、inferType、404/410）。
- [ ] `prepareVoiceTools`/`loadAudioSource`/`detectAudioMimeType`/`prepareModelToolChanges`/路径穿越守卫零测试覆盖。
- [ ] 缺 `FallbackChatModelTest`（休眠分支行为）。

### 20.7 验证中被否决的两个疑似 bug（定论）

1. "热更新绑定旧 provider" — 否：工具持 `VoiceDelegate`，调用时取当前 generation。
2. "`.audio` 文件名标记是死代码" — 否：它是 `isAudioFile` 识别录音的关键契约（`AgentPromptBuilderTest:79-92` 钉住）。

### 20.8 修复顺序（执行时从此处取）

1. 鉴权（artifacts + upload + /files，需先定无 token 播放方案）——**暂缓（2026-09-10 用户决定）：当前部署不需要；后期如需要，在接口加拦截层即可。**
2. 上传白名单补 m4a/flac/aac + Safari 扩展名修正。
3. 畸形工具参数不得取消整个 SSE 流。
4. sessionId 绑定与 artifact 保留/GC。
5. 提示语与工具可用性对齐。
6. Realtime 死链清理。
7. 其余 Minor 与测试补齐。
