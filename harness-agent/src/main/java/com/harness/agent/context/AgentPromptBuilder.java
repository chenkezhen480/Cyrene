...
public final class AgentPromptBuilder {
    ...
    public String buildSystemPrompt(
        ...
        prompt.append("When memory tools are available, capture useful durable knowledge; never invent an entry "
                + "or require every kind. Use save_operation_playbook for a reusable Agent method learned from "
                + "observed work: conditions, steps and verification, without user-specific facts. Use "
                + "save_user_preference for a lasting personal habit or response constraint explicitly expressed "
                + "by the user. If the user asks how this Agent should answer them, that is a preference; "
                + "a procedure the Agent learned for similar work is a playbook. Use save_user_episode for a "
                + "concrete event that happened to the user, with context and outcome. Call separately for "
                + "each applicable kind. Do not save guesses, temporary chatter, secrets or raw tool results. "
                + "Preferences are saved only in MySQL and injected before later model calls; they are not "
                + "Wiki/vector memories. Custom preference keys need activationTags. Never infer preferences "
                + "from uploaded document content alone. Saved means committed; pending means indexing is "
                + "asynchronous.\n\n");
