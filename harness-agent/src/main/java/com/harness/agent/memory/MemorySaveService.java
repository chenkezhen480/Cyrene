...
public final class MemorySaveService {
    ...
    public MemorySaveService(KnowledgeRepository repository, ObjectMapper mapper,
    ...
    String save(KnowledgeConceptType type, JsonNode arguments, String toolName) {
        try {
            KnowledgeToolRuntimeContext context = KnowledgeToolRuntimeContext.requireCurrent(toolName);
            if (!context.authorizedTools().contains(toolName)) {
            ...
            KnowledgeRevision revision = new KnowledgeRevision(
                    ...
                    version + 1, title, summary, content, toolName, now, hash, metadata, now);
            ...
            throw new ToolExecutionException(toolName, "Memory save failed: " + exception.getMessage());
        }
    }
    ...
    static String text(JsonNode arguments, String field, int limit) {
